...
export default function ShowUserArticles() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3000/userArticles`)
      .then((response) => {
        let art = response.data;
        let art2 = art.filter(
          (article) => Number(article.postedBy) === Number(userId)
        );
        setArticles(art2);
        console.log(art2);
      })
            ...
            <div className="h-[554px]  ml-[31px] mr-[44px] flex flex-col relative  font-inter">
              {articles.map((post) => (
                <div key={post.id}>
                  ...
                  >
                    <div className="bg-gray-400 border-2  max-w-[328px] h-[201px]">
                      <img
                        src={post.imgUrl}
                    ...
                    <div className="w-full h-full  flex flex-col">
                      <div className="h-[50px] w-full flex justify-between items-center pr-1 text-white">
                        ...
                        <div className="flex gap-x-1">
                          <div className="bg-[#D9D9D9] h-[27px] w-[27px] rounded-full hidden"></div>
                          <h1 className="hidden">{post.user.username} </h1>
                        </div>
                      </div>
